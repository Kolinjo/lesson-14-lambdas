...
class _2_PracticeWritingLambdas {
	...
	Function<Integer, Integer> INCREMENT_LAMBDA = x -> ++x ;
	Function<String, String> REMOVE_VOWELS_LAMBDA = string -> string.replaceAll("[aeiou]", "") ;
	Function<Integer, Integer> DOUBLE_LAMBDA = x -> x + x ;
	BiFunction<Integer, Integer, Integer> PYTHAGOREAN_LAMBDA = (x,y) ->(int) Math.sqrt(x*x+y*y);
	Function<Integer, String> EVEN_ODD_LAMBDA = x -> {
		if (x % 2 == 0 ) {
			return "even";
		}else
			return "odd";
